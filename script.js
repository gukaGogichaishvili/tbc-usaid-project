...
//ჰედერი სქროლზე ხდება გამჭირვალე 
...
function updateCarousel() {
  ...
  setTimeout(() => {
    ...
    for (
      let i = currentIndex;
      i < currentIndex + 3 && i < imageUrls.length;
      i++
    ) {
...
function moveLeft() {
  ...
  currentIndex =
    currentIndex - 3 >= 0
      ? currentIndex - 3
      : Math.max(0, imageUrls.length - (imageUrls.length % 3));
  updateCarousel();
}
...
const accordionData = [
  {
    ...
    answer:
      "<span>კურსზე რეგისტრაციისთვის უნდა გაიარო რამდენიმე ეტაპი:</span><br/> <p><b>I ეტაპი</b><span> - პირველ ეტაპზე საჭიროა, შეავსო სასურველი კურსისთვის განკუთვნილი სარეგისტრაციო ფორმა, რომელიც განთავსებულია კურსის შიდა გვერდზე. კურსზე რეგისტრაციის დასრულების შემდეგ დაიწყება შემოსული განცხადებების გადარჩევა.</span></p>" +
...
function renderAccordion(accordionItems) {
  ...
  accordionItems.forEach((item, index) => {
    ...
    accordionHeader.innerHTML =
      item.question +
      '<span class="material-icons small-arrow">keyboard_arrow_down</span>';
    ...
    accordionHeader.onclick = () =>
      toggleAccordion(accordionContent, accordionHeader);
...
//კურსების ქარდების გრიდი
const cardData = [
  {
    imgSrc: "img/card1.jpg",
    title: "iOS Development",
    text: "რეგისტრაცია დასრულებულია",
  },
